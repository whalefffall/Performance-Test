...
for((var=0; var<$warmUp; var++))
    do
        { time -p ./perlbench_r_base.mytest-m64 -I. -I./lib makerand.pl > /dev/null 2>&1;} >> 1.out 2>&1
done
...
for((var=0; var<$warmUp; var++))
    do
        { time -p ./perlbench_r_base.mytest-m64 -I. -I./lib test.pl > /dev/null 2>&1;} >> 1.out 2>&1
done
echo finish one warm up!
