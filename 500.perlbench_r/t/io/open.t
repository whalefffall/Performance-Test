...
SKIP: { skip "SPEC CPU: No forking pipes", 3;
...
SKIP: { skip "SPEC CPU: No forking pipes", 5;
    ok( open(my $f, '|-', <<EOC),     'open |-' );
    $Perl -pe "s/^not //"
EOC
...
SKIP: { skip "SPEC CPU: No forking pipes", 12;
    local *F;
    for (1..2) {
...
SKIP: { skip "SPEC CPU: No fresh perls", 2;
...
SKIP: { skip "SPEC CPU: No forking pipes", 1;
    package p73626;
    sub TIESCALAR { bless {} }
    sub FETCH { "$Perl -e 1"}
...
SKIP: { skip "SPEC CPU: No fresh perls", 1;
fresh_perl_is(
    '
      open my $fh, ">", \*STDOUT;
      print $fh "hello";
...
# [perl #117265] check for embedded nul in pathnames, allow ending \0 though
{
    ...
    close $temp_fh;
SKIP: { skip "SPEC CPU: No utime() support";
    ...
    SKIP: {
        skip "SPEC CPU: No utime() or chown(), and maybe not Errno", 8;
