...
SKIP: {
    my $restore_root;
    skip "SPEC CPU: No need to test -w test for the benchmark", 1;
...
for my $op (split //, "rwxoRWXOezsfdlpSbctugkTMBAC") {
	...
	||
	# SPEC CPU: S_ISVTX isn't available on Win32
	( $::is_spec_cpu_windows && $op eq 'k' )
    ) {
        ($exp, $is) = (0, "not");
    }
...
SKIP: {
    skip "SPEC CPU: Don't worry about -T", 8;
