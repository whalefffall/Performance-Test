...
# SPEC CPU: no times, and no time for a 20+ second test
if (0) {
...
# SPEC CPU: no times, and no time for a 20+ second test
SKIP: { skip "SPEC CPU: No times", 1;
my $x = "aaaa";
for ($i = 0; $i < 1_000_000; $i++) {
    for my $j (1..1000) {}; # burn some user cycles
...
SKIP: {
    ...
    skip "No tzset()", 1
        if $^O eq "VMS" || $^O eq "cygwin" ||
           ...
           $^O eq "interix" || 1; # SPEC CPU: No env vars
...
SKIP: { #rt #73040
    skip "500.perlbench doesn't use NV for time values", 8;
