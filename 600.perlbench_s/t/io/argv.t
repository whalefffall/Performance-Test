...
if (1 || is_miniperl()) { # SPEC CPU: No dynamic loading here either
...
SKIP: { skip "No running perl in SPEC CPU tests", 8;
$x = runperl(
    prog	=> 'while (<>) { print $., $_; }',
...
@ARGV = 1||is_miniperl() ? ('Io_argv1.tmp', 'Io_argv1.tmp', 'Io_argv1.tmp') # SPEC CPU can't count on /dev/null
...
SKIP: { skip "SPEC CPU: in-place editing is busted on Windows", 3 if $::is_spec_cpu_windows;
...
SKIP: { skip "SPEC CPU: The aforementioned in-place editing wasn't done", 2 if $::is_spec_cpu_windows;
...
SKIP: {
    ...
    skip $no_devnull, 4; # SPEC CPU is like miniperl, sorta
...
SKIP: {
    ...
    skip $no_devnull, 5; # SPEC CPU is like miniperl, sorta
...
SKIP: { skip "SPEC CPU: no fresh perls", 11;
open(TRY, '>Io_argv1.tmp') || (die "Can't open temp file: $!");
print TRY "one\n\nthree\n";
close TRY or die "Could not close: $!";
