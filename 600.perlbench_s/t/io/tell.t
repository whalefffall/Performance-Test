...
$Is_Dosish = ($^O eq 'MSWin32' or $^O eq 'NetWare' or $^O eq 'dos' or
              ...
              $^O =~ /^uwin/);
$Is_Dosish ||= $::is_spec_cpu_windows;

open($TST, 'test.pl') || (die "Can't open test.pl"); # SPEC CPU: no 'harness' file
...
$curline = $.;
open(OTHER, 'test.pl') || (die "Can't open test.pl: $!"); # SPEC CPU: No 'harness' file
