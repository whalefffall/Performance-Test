...
SKIP:
{
$tests += 8; skip "SPEC CPU: No network calls", 8;
...
SKIP:
{
skip "SPEC CPU: No network calls", 3;
...
test_proto 'chown';
$tests += 4;
SKIP: { skip "SPEC CPU: chown is not supported", 4;
...
test_proto 'closedir';
$tests += 2;
SKIP: { skip "SPEC CPU: No directory reading functions", 2;
...
SKIP:
{
skip "SPEC CPU: No network functions", 2;
...
test_proto 'exit';
$tests ++;
SKIP: { skip "SPEC CPU: No tests involving external processes";
...
SKIP: { $tests++; skip "SPEC CPU: No signal stuff either";
if ($^O ne 'riscos') {
    $tests ++;
    ok( &mykill(0, $$), '&kill' );
}
...
unless ($::is_spec_cpu_windows or $^O eq "MSWin32" && is_miniperl) {
...
test_proto 'time';
$tests += 2;
SKIP: { skip "SPEC CPU: No internal timing as well", 2;
...
next if $word =~ /^(accept|bind|chown|closedir|connect|exit|kill|time|times)$/; # SPEC CPU
      $tests ++;
      ok   exists &{"my$word"}
...
{
  last if 'SPEC CPU';   # Cwd can fork/exec; File::Temp does directory ops; etc.
  last if is_miniperl;
  require Cwd;
  import Cwd;
  $tests += 3;
