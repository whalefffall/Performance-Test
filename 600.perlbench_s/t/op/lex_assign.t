...
$aref = \@a;
$href = \%h;
# Not in SPEC CPU: open OP, qq{$runme -le "print 'aaa Ok ok' for 1..100"|};
$chopit = 'aaaaaa';
@chopar = (113 .. 119);
$posstr = '123456';
$cstr = 'aBcD.eF';
...
ref $xref			# ref
ref $cstr			# ref nonref
`$runme -e "print qq[1\\n]"`				# backtick skip(MSWin32) skip(spec-cpu)
`$undefed`			# backtick undef skip(MSWin32) skip(spec-cpu)
<*>				# glob
<OP>				# readline skip(spec-cpu)
'faked'				# rcatline
(@z = (1 .. 3))			# aassign
(chop (@x=@chopar))		# chop
chop $chopit			# schop
...
select undef,undef,undef,0	# select skip(spec-cpu)
getc OP				# getc skip(spec-cpu)
'???'				# read
'???'				# sysread
'???'				# syswrite
'???'				# send
...
'???'				# wait
'???'				# waitpid
system "$runme -e 0"		# system skip(VMS) skip(spec-cpu)
'???'				# exec
'???'				# kill
getppid				# getppid
getpgrp				# getpgrp
