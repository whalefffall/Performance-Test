...
$Is_Dos     = $^O eq 'dos';
$Is_MSWin32 = $^O eq 'MSWin32' || $::is_spec_cpu_windows;
...
SKIP: {
    skip "SPEC CPU: No forking tests", 6;
...
skip "SPEC CPU: No directory reading support", 6;
...
SKIP: {
skip "SPEC CPU: No directory reading functions", 3;
    my($cnt, $uid);
    $cnt = $uid = 0;
...
SKIP: {
  skip "SPEC CPU: No TTY tests in the harness", 4;
...
ok(  -T $statfile,    '-T');
ok(! -B $statfile,    '!-B');
SKIP:{ skip "SPEC CPU: No tests on the benchmark binary", 2;
ok(-B $Perl,      '-B');
ok(! -T $Perl,    '!-T');
} # !SPEC CPU
...
SKIP: {
skip "SPEC CPU: No directory reading support", 9;
