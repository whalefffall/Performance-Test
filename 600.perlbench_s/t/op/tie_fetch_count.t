...

{
    ...
    no warnings;
SKIP: { skip "SPEC CPU: No directory reading functions";
...
SKIP: { skip("SPEC CPU: No support for ${op}()", 2) if ($op eq 'chown' || $op eq 'utime');
...
SKIP: { skip "SPEC CPU: No crypt() in the benchmark", 4;
...
SKIP:
{
    skip "SPEC CPU: no select in the benchmark", 3;
