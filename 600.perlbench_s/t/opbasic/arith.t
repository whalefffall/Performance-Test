...
if ('SPEC CPU') {
  # SPEC CPU does not require strict IEEE-754 compliance, so flush-to-zero is okay.
  try $T++, 1, 'min double flush-to-zero ignored for SPEC CPU';
} else { # SPEC CPU
# For flush-to-zero systems this may flush-to-zero, see PERL_SYS_FPU_INIT
try $T++,  2.2250738585072014e-308 != 0.0, 'min double';
} # End SPEC CPU exceptions
