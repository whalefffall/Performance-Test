...
# For SPEC CPU, figure out if this is Windows.  $^O can't be used; it's a
# generic "spec-cpu".  But there are still some times when we must know
# it's Windows, and this seems like a reasonable way to guess.
$::is_spec_cpu_windows = (grep { $_ eq 'crlf' } PerlIO::get_layers(STDOUT)) + 0;
...
sub _diag {
    return unless @_;
    my @mess = _comment(@_);
    1 || $TODO ? _print(@mess) : _print_stderr(@mess);  # For SPEC CPU, all to stdout please
}
...
sub runperl {
  return 'SPEC CPU: no test involving external processes'; # No runperl tests for SPEC CPU
...
sub which_perl {
	...
	$Perl = $^X;
        ...
        return $Perl; # For SPEC CPU, $^X is always correct
...
sub watchdog ($;$)
{
  return; # SPEC CPU: No alarm, no kill, no fork == no watchdog
    my $timeout = shift;
    my $method  = shift || "";
