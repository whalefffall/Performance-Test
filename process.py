...
roles = ['real', 'user', 'sys']
log_file_sev_disable = "2.out" # sev disable log file name
log_file_sev_enable = "1.out" # drv enablelog file name
...
with open(os.path.join(root, result_txt), "w") as r_txt, open(os.path.join(root, result_csv), "w") as  r_csv:
    ...
    for dir in dirs:
        ...
        with open(os.path.join(cur_path, log_file_sev_enable), "r") as log:
            lines = log.readlines()
            ...
            while idx < len(lines):
                if e_cnt == execute:
                    sev_enable_warmup_list.append([c_cnt, warmup_real_all / warmUp, warmup_usr_all / warmUp, warmup_sys_all / warmUp])
                    sev_enable_execute_list.append([c_cnt, execute_real_all / execute, execute_usr_all / execute, execute_sys_all / execute])
        ...
        with open(os.path.join(cur_path, log_file_sev_disable), "r") as log:
            lines = log.readlines()
            c_cnt = 1 # command number
            w_cnt = 0 # warm up count
            e_cnt = 0 # execute count
            ...
            while idx < len(lines):
                if e_cnt == execute:
                    sev_disable_warmup_list.append([c_cnt, warmup_real_all / warmUp, warmup_usr_all / warmUp, warmup_sys_all / warmUp])
                    sev_disable_execute_list.append([c_cnt, execute_real_all / execute, execute_usr_all / execute, execute_sys_all / execute])
        ...
        for cnt in range(len(sev_disable_execute_list)):
            ...
            # for execute
            r_csv.write("%s,%d,%s,%d,%s,%f,%f,%f,\n"%(dir, cnt+1, "execute", execute, roles[0], sev_enable_execute_list[cnt][0], sev_disable_execute_list[cnt][0], sev_enable_execute_list[cnt][0]/sev_disable_execute_list[cnt][0]))
