...
with open(os.path.join(root, result_txt), "w") as r_txt, open(os.path.join(root, result_csv), "w") as  r_csv:
    ...
    for cnt in range(len(sev_disable_execute_list)):
        ...
        r_csv.write("%s, %s, %s, %d, %f, %f, %f"%(sev_disable_warmup_list[cnt][0], sev_disable_warmup_list[cnt][1], "warmup", warmUp, sev_enable_warmup_list[cnt][2], sev_disable_warmup_list[cnt][2], sev_enable_warmup_list[cnt][2] / sev_disable_warmup_list[cnt][2]))
        r_csv.write("%s, %s, %s, %d, %f, %f, %f"%(sev_disable_execute_list[cnt][0], sev_disable_execute_list[cnt][1], "execute", execute , sev_enable_execute_list[cnt][2], sev_disable_execute_list[cnt][2], sev_enable_execute_list[cnt][2] / sev_disable_execute_list[cnt][2]))